...
import {
   ...
   restaurantWrongCreateBodyMock,
} from "../../__mocks__/restaurant.mocks";
import { request } from "../../utils/request";

describe("Integration test: register restaurant", () => {
   it("should be able to register a restaurant successfully", async () => {
   ...
   it("should throw error when email is already registered", async () => {
      ...
      await request.post("/restaurants").send(restaurantCreateBodyMock).expect(403);
   });
