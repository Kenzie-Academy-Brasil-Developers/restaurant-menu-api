...
export class RestaurantControllers {
   constructor(
      @inject("RestaurantServices") private restaurantServices: RestaurantServices
   ) {}
   ...
   async getRestaurant(
      req: Request,
      res: Response
   ): Promise<Response<TRestaurantReturn>> {
      const { id } = res.locals.decode;
   ...
   async getManyRestaurants(
      req: Request,
      res: Response
   ): Promise<Response<TPublicRestaurant[]>> {
